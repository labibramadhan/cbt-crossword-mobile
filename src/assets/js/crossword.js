...
function Crossword(words_in, clues_in) {
  ...
  // var words_length = _.reduce(_.map(words_in, function (w) {
  //   return w.length
  // }), function (a, b) {
  //   return a + b;
  // });
  // var words_offset = _.reduce(_.map(words_in, function (w) {
  //   return Math.ceil(w.length / 2) + 3
  // }), function (a, b) {
  //   return a + b;
  // });
  // var GRID_ROWS = Math.ceil((words_length + words_offset) / GRID_COLS);
  var GRID_ROWS = 500;
  ...
  this.getSquareGrid = function (max_tries) {
    grid_reshuffle = max_tries;
    var best_grid = null;
    var best_ratio = 0;
    for (var i = 0; i < max_tries; i++) {
      var a_grid = this.getGrid(5);
  ...
  var findPositionForWord = function (word) {
    ...
    if (bests.length == 0) {
      ...
      for (var t = 0; t < 100; t++) {
        ...
        var rr = _.random(GRID_ROWS);
        var rc = _.random(GRID_COLS);
        if(_.indexOf(random_hits, rr.toString().concat(rc.toString())) > -1) continue;
        if (typeof(grid[rr]) === "undefined" || grid[rr][rc] !== null) continue;
        random_hits.push(rr.toString().concat(rc.toString()));
        var rd = _.shuffle(['across', 'down'])[0];
        var intersections = canPlaceWordAt(word, rr, rc, rd);
        if (intersections !== false) {
          bests.push({"intersections": 0, "row": rr, "col": rc, "direction": rd})
        }
      }
...
var CrosswordUtils = {
  ...
  getLegend: function (words_in, clues_in, ids, grid) {
      ...
      for (var c = 0; c < grid[r].length; c++) {
        ...
        for (var k in groups) {
          ...
          if (cell && cell[k] && cell[k]['is_start_of_word']) {
            ...
            switch (k) {
              case "across":
                endCol = c + words_in[index].length - 1;
                endRow = r;
                break;
              case "down":
                endCol = c;
                endRow = r + words_in[index].length - 1;
                break;
            }
            groups[k].push({
              "position": position,
  ...
  findLegendByNumber: function (legend, number) {
    var cell = [];
    var types = ['across', 'down'];
    _.each(types, function (type) {
      _.each(legend[type], function (c) {
        if (c.position === number) {
          cell.push(c);
        }
      });
    });
  ...
  findLegendByCell: function (legend, row, col, cell) {
    var cells = [];
    var types = ['across', 'down'];
    _.each(types, function (t) {
      _.each(legend[t], function (c) {
        if (c.position === cell.position && c.type === cell.type) {
          return;
        }
        var findRow = _.indexOf(_.range(c.startRow, c.endRow + 1), row) > -1;
        var findCol = _.indexOf(_.range(c.startCol, c.endCol + 1), col) > -1;
        if (findRow && findCol) {
          cells.push(c);
        }
      });
